...
names = ["Arnetta", "Rahul", "Henry", "Shayak", "Rishi", "Katie", "Jenna", "Ryan", "Sarah", "Brad", "Mike", "Jay", "Esteban", "Chad", "Ben", "Surya", "Wally", "Evan", "Eric", "Jack", "Jordan"]
rand = names[Math.floor(Math.random() * names.length)];
...
function buildProfileSongs() {
    ...
    if (songList != null) {
        for (var i = 0; i < mySongs["items"].length; i++) {
            ...
            var inner_html = `
<td width=5%><img class="album_art" src="images/cbook.jpg"></img></td>
<td>${mySongs["items"][i]["track"]["name"]}</td>
<td>Single</td>
<td>${artists_string}</td>
`;
...
function saveToSessionStorage(addedSong) {
    var mySongs = [];
    mySongs = JSON.parse(sessionStorage.getItem('session'));
